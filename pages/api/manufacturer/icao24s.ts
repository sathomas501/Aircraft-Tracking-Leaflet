...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        // Handle GET and POST request parameters
        if (req.method === 'GET') {
        ...
        // Build SQL query with optional model filtering
        let query = `
        ...
        query += ' LIMIT 500'; // Add reasonable limit for performance
        const aircraft = await db.all(query, params);

        res.status(200).json({
            icao24List: aircraft.map((a: { icao24: string }) => a.icao24),
            meta: {
