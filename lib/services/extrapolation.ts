import { Position } from '../services/tracking-services/aircraft-position-service';
...
export function shouldUpdatePosition(
  ...
  const Δφ =
    ((newPosition.latitude - currentPosition.latitude) * Math.PI) / 180;
  const Δλ =
    ((newPosition.longitude - currentPosition.longitude) * Math.PI) / 180;
