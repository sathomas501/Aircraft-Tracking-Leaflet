...
const modelsCache: Record<string, { models: AircraftModel[]; timestamp: number }> = {};
...
export function useAircraft({
  ...
  const fetchManufacturers = useCallback(async () => {
    ...
    try {
      const response = await dedupedRequest('GET-manufacturers', async () => {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to load manufacturers';
  ...
  const fetchModels = useCallback(async (manufacturer: string | null) => {
    if (!manufacturer) {
      setModels([]);
      return [];
    }
    ...
    try {
      const response = await dedupedRequest(`GET-models-${manufacturer}`, async () => {
  ...
  const startTrackingAircraft = useCallback(async (manufacturer: string) => {
    ...
    try {
      ...
      const trackResult = await dedupedRequest(`track-init-${manufacturer}`, async () => {
      ...
      if (trackResult.count > 0) {
        updateStatus(`Initialized tracking for ${trackResult.count} aircraft`);
      ...
      const errorMsg = err instanceof Error ? err.message : 'Failed to initialize tracking';
      handleError(errorMsg, err);
    } finally {
      setIsLoading(false);
      abortControllerRef.current = null;
  ...
  const fetchLivePositions = useCallback(async (manufacturer: string) => {
    if (!manufacturer) return;
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Error connecting to OpenSky';
      handleError(errorMsg, err);
    }
  }, [updateStatus, handleError]);

  // 5. Process aircraft data into models
  const processAircraftModels = useCallback((aircraftData: Aircraft[]) => {
    ...
    aircraftData.forEach((aircraft: Aircraft) => {
      ...
      if (existing) {
        ...
        existing.activeCount = (existing.activeCount || 0) + (aircraft.isTracked ? 1 : 0);
  ...
  const startPolling = useCallback((manufacturer: string) => {
  ...
  const subscribeToClient = useCallback((manufacturer: string | null) => {
  ...
  const mergeModels = useCallback(() => {
    ...
    const mergedModels = Array.from(modelMap.values()).sort((a, b) => {
      ...
      const totalCountDiff = (b.totalCount || b.count || 0) - (a.totalCount || a.count || 0);
  ...
  const selectManufacturer = useCallback(async (manufacturer: string | null) => {
  ...
  const totalFilteredAircraft = useMemo(() => filteredAircraft.length, [filteredAircraft]);
